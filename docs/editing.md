...
the document in the middle, the rendered page on the right — and the thing worth understanding
before anything else is what it writes.
...
!!! warning "`kineglyph` is deliberately not bundled"
    ...
    specifier* — a configuration requirement, not a bug. React **is** bundled: a host page must not
    have to install it.
...
| Admonition | `!!! note "Title"` — `???` when collapsible, indented body |
...
- **Versions are a monotonic counter, not a content hash.** Rewriting identical bytes still moves
  ...
  version is never reused — a stale one cannot accidentally match a file that was deleted and
...
`publish()` is accepted and the payload kept in memory, but only the timestamp is persisted — a
...
- **A page cannot `--8<--` its own `article.yaml`.** The build succeeds and `pagina pack` then fails
  with `[bundle-collision] two different files both want to be article.yaml in the bundle` — `pack`
