...
import type { OgScheme } from "@pagina/core";
...
/** Reads a stylesheet the article ships. A URL is not a file, and says so. */
async function readTheme(folder: string, rel: string, from: string, diagnostics: Diagnostic[]): Promise<string | undefined> {
  if (/^(?:[a-z][a-z0-9+.-]*:|\/\/)/i.test(rel)) {
    diagnostics.push({
      ...
      message: `${from} is a URL (${rel}), and a social card's palette is baked from files at build time — the card uses the tokens this build can read. Ship the stylesheet in the article folder to theme its cards.`,
    });
    return undefined;
  }
  try {
    return await readFile(resolve(folder, rel), "utf8");
  } catch {
    diagnostics.push({
      severity: "warning",
      code: "og-theme-missing",
      message: `${from} names ${rel}, which is not in the article folder — the card keeps pagina's palette.`,
    });
    return undefined;
  }
}
...
export async function resolveCardPalette(
  ...
  if (sources.articleTheme !== undefined) {
    const css = await readTheme(folder, sources.articleTheme, "article.yaml's `theme`", diagnostics);
    if (css !== undefined) palette = applyTokens(palette, readPgTokens(css, scheme), "article.yaml's `theme`", diagnostics);
  }
  if (sources.pageTheme !== undefined) {
    // A page's `theme:` is relative to the page, the way its `cover:` is.
    const dir = sources.pagePath === undefined ? "." : `${sources.pagePath}/..`;
    const css = await readTheme(resolve(folder, dir), sources.pageTheme, "the page's `theme`", diagnostics);
    if (css !== undefined) palette = applyTokens(palette, readPgTokens(css, scheme), "the page's `theme`", diagnostics);
  }
